...
container.innerHTML += `
<div class="product">
<img data-src="${p.imagen}" class="lazy">
<h3>${p.nombre}</h3>
<p>€${p.precio}</p>
...
newImg.onload = ()=>img.classList.add("loaded");
img.src = newImg.src;
});
}
...
function changeQty(nombre,delta){const item=cart.find(p=>p.nombre===nombre); item.cantidad+=delta; if(item.cantidad<=0){removeItem(nombre); return;} saveCart(); updateCart();}
function updateCart(){
